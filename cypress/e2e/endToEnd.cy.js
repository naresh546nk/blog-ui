...
describe("Login and adding new blogs and then view", () => {
  it("passes", () => {
    ...
    cy.findByRole("button", { name: /you blogs/i })
      .its("length")
      .should("be.gte", 0);
    cy.findByRole("button", { name: /other blogs/i })
      .its("length")
      .should("be.gte", 1);
    cy.findByRole("button", { name: /all blogs/i })
      .its("length")
      .should("be.gte", 1);
    ...
    cy.get(".d-flex > :nth-child(3) > span")
      .invoke("text")
      .then((text1) => {
        cy.get(".d-flex > :nth-child(1) > span")
          .invoke("text")
          .then((text2) => {
            cy.get(".d-flex > :nth-child(2) > span")
              .invoke("text")
              .then((text3) => {
    ...
    cy.wait(2000);
    cy.get(".me-2").type("Motivation is the part of our life");
    ...
    //Login as admin
    cy.findByRole("textbox", { name: /email/i }).type("naresh546.nk@gmail.com");
    cy.findByLabelText(/password/i).type("Naresh@123");
    cy.findByRole("button", { name: /login/i }).click();
    cy.wait(5000);

    //blogs created by others
    cy.get(".fixed-top > .d-flex > :nth-child(2)").click();
    //clicking on first blogs of other
    cy.get(
      ":nth-child(1) > .text-decoration-none > .boxShadow > .card-header"
    ).click();
    cy.wait(1000);
    //checking if button is enable.
    cy.get("div.m-2 > .btn").should("not.be.disabled");
  });
});
